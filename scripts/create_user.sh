...
echo "Creating user ( UIDNUMBER=$UIDNUMBER NAME=$NAME SURNAME=$SURNAME EMAIL=$EMAIL USERNAME=$USERNAME PASS=$PASS LIB_PATH=$LIB_PATH ) in LDAP" >> /tmp/ldap.log
...
sudo /usr/sbin/service proftpd restart
