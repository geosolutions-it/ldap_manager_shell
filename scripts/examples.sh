...
ldapsearch -x -h localhost -p 389 -D "cn=admin,dc=mariss,dc=egeos,dc=it" -w theslap -b "ou=groups,dc=mariss,dc=egeos,dc=it"

# Printing all the users
ldapsearch -x -h localhost -p 389 -D "cn=admin,dc=mariss,dc=egeos,dc=it" -w theslap -b "ou=people,dc=mariss,dc=egeos,dc=it"
