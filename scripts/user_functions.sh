...
function modify_ldap_user_conf_no_pass {
  cat <<- EOF
dn: uid=$USERNAME,ou=people,dc=mariss,dc=egeos,dc=it
changetype: modify
replace: cn
...
function usage {
  echo "USAGE:"
  echo "      UIDNUMBER="UIDNUMBER" USERNAME="THEUSERNAME" NAME="THENAME" SURNAME="THESURNAME" EMAIL="THEMAIL" PASS="THEPASS" [LOGFILE="THELOGFILE.log"] $0"
...
function check {
  if [ -z $USERNAME ]; then
    echo "check USERNAME"
    usage
  elif [ -z $UIDNUMBER ]; then
    echo "check UIDNUMBER"
    usage
  elif [ -z $NAME ]; then
    echo "check NAME"
    usage
